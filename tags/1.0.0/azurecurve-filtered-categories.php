...
function azc_fc_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        $settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=azc_fc">Settings</a>';
...
function process_azc_fc() {
	...
	if ( ! empty( $_POST ) && check_admin_referer( 'azc_fc_nonce', 'azc_fc_nonce' ) ) {
		...
		wp_redirect( add_query_arg( 'page', 'azc_fc&options-updated', admin_url( 'options-general.php' ) ) );
		exit;
	}
}
